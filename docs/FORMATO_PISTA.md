...
- **sources**: lista di `source_dataset` + `source_record_id` + `source_url`
...
- **data_through**: data di copertura dei dati (derivata dal periodo massimo nei dati; non è la data di generazione)
- **snapshot_created_at**: data di creazione dello snapshot sorgente (derivata da `acquisition_date` dei dati; non l'orario di esecuzione)
- **explorer_sha**: hash SHA-256 dei dati di input dell'Explorer (identifica lo snapshot esatto)
